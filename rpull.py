...
def recursive_pull(list_of_repo: list, root_dir: str):
    for repo in list_of_repo:
        print('%+50s' % f'\n\n\n=====_update {repo}_=====')
        os.system(f'cd {os.path.join(root_dir, repo)}&& git checkout master && git pull')
...
def main():
    ...
    if len(gits) == 0:
        ...
        input()
    recursive_pull(list_of_repo=gits, root_dir=path)
    print('Done')
